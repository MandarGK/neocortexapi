...
namespace NeoCortexApiExperiment
{
    ...
    public class SpatialLearningExperiment
    {
        ...
        private static SpatialPooler RunExperiment(HtmConfig cfg, EncoderBase encoder, List<double> inputValues, int numColumns)
        {
            ...
            for (int cycle = 0; cycle < maxSPLearningCycles; cycle++)
            {
                ...
                foreach (var input in inputs)
                {
                    ...
                    Debug.WriteLine($"[cycle={cycle.ToString("D4")}, N={stableIterationCount[Convert.ToInt32(input)]} , stablecycles ={stableCycleElapsed}, i={input}, cols={actCols.Length} s={similarity}] SDR: {string.Join(", ", SdrDictionary[Convert.ToInt32(input)].Last())}");
        ...
        {
            Debug.WriteLine("***************************************************************************************************************\n");
            Debug.WriteLine($"Printing the SDRs of 100 consecutive stablecycles after achieving the stable state:\n");
        ...
        private static void PrintSdrsForInput(Dictionary<int, List<List<int>>> sdrDictionary, List<(int Cycle,double Similarity, int[] ActCols,List<int> SDR)> sdrsForInput0)
        {
            ...
            {
                Debug.WriteLine($"Input 0, Cycle: {cycle}, s: {similarity}, Columns: {actCols.Length}, SDR: {string.Join(", ", sdr)}");
            }
